...
import {AceptarCookiesReducer, LeyendoCookiesReducer} from '../../Redux/Acciones/Setting'
import {Link} from "react-router-dom";
import config from '../../config'
import { Switch, Modal } from 'antd';
import {CheckCircleTwoTone } from '@ant-design/icons';
import '../../Estilos/Rutas/Terminos/Terminos.css'
import IconoEquisAzul from '../../Assets/Img/CargaArchivos/equisazul.svg'
import GifSwitch from '../../Assets/Gif/switch.gif'
...
const Terminos = () => {
    ...
    useEffect(() => {
        ...
        if(cargando_vista_inicio_sistema == true){
            setMostrarDirectamenteInstructivo(false)
        }else{
            if(mostrarDirectamenteInstructivo == true){
                setMostrarModalInstructivo(true)
            }
            
        }
        
        if(mostrarDirectamenteInstructivo == false){
            if(cargando_vista_inicio_sistema == false){
                setTimeout(() => {
                    setMostrarModalInstructivo(true)
                }, 1000);
            }
    ...
    const [mostrarModalInstructivo, setMostrarModalInstructivo] = useState(false)
    const [mostrarDirectamenteInstructivo, setMostrarDirectamenteInstructivo] = useState(true)

    

    return (
        <div style={{paddingLeft:'40px', paddingRight:'40px', paddingTop:'40px', paddingBottom:'40px', fontFamily:'Montserrat, sans-serif'}} >
            ...
            {
                cargando_vista_inicio_sistema == false
                ?datosUsuarioLogeado.usuaceptoterminos
                ?null
                :<Modal
                    ...
                    closeIcon={
                        <div 
                            style={{
                                borderRadius:'30px', background:'#E7F3FF', width:'35px', height:'35px', marginTop:'10px',
                                position: "relative",
                                // right: "10px"
                            }}
                        ...
                        >
                            <img src={IconoEquisAzul} 
                                style={{
                                    width:'35px', position:'absolute', left:'0', 
                                    top: "2px"
                                }} 
                            />
                        </div>
                    ...
                    width="365px"
                    height="183px"
                    className='Modales-Terminos-Condiciones'
                >
            ...
            <Modal
                ...
                closeIcon={
                    <div 
                        style={{borderRadius:'30px', background:'#E7F3FF', width:'30px', height:'30px', position:'absolute', marginTop:'10px', right:'10px'}}
                        onClick={() => setMostrarModalWarning(false)}
                    >
                        <img src={IconoEquisAzul} style={{width:'30px', position:'absolute', left:'0', top:'1px'}} />
                    </div>
                }
                width="365px"
                height="183px"
            ...
            >
                ...
                >
                    {/* Para continuar usted tiene que aceptar todos los switches de los subtítulos en la pestaña de términos y condiciones */}
                    <div style={{whiteSpace: "nowrap"}}>Para aceptar los <b>Términos y Condiciones</b> deberá<br/> activar <b>todos</b> los <b>Switches</b> de cada subtítulo</div>
                </div>
